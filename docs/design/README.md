...
Використовується для зберігання даних облікового запису користувача, які вводяться під час його реєстрації, і з якими система звіряє введені під час авторизації дані.

Має поля:

- login: TEXT. Містить текст із логіном користувача;

- email: TEXT. Містить текст із адресою електронної пошти користувача;

- password: TEXT. Містить текст із паролем користувача.

  <a name="member"></a> 

### Member (Учасник)

Використовується як посередник; визначає те, які [права](#grant) певний [користувач](#user) має у доступі до певного [проєкту](#project).
...
Використовується для роботи з проєктами.
Має поля:

- name: TEXT. Містить текст з назвою проєкту.
- description: TEXT. Містить текст з описом проєкту.

Проєкт може мати від 0 до * [розділів](#section) та від 0 до * [учасників](#member).
Користувачі можуть мати різні [ролі](#role) в проєкті, та виконувати різні дії відповідно до них. Коли [користувач](#user) створює проєкт, йому автоматично надається [роль](#role) "Замовник", що визначає максимальні [права](#grant).
